...
export function Suppliers() {
  ...
  const fetchData = () => {
    setLoading(true);
    apiClient.getSuppliers(category, productName, minVolume ? parseInt(minVolume) : undefined, startDate, endDate)
      .then(res => {
        setData(res);
        setLoading(false);
      });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          <div>
            ...
            <input type="text" value={category} onChange={e => setCategory(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Напр. Фірма" />
          </div>
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">Вид товару</label>
            <input type="text" value={productName} onChange={e => setProductName(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Напр. Гальма" />
          </div>
          <div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-slate-800">Результати</h2>
              ...
              <tbody className="divide-y divide-slate-100">
                {data.map(supplier => (
                  <tr key={supplier.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4 text-slate-600">{supplier.contactInfo}</td>
                  </tr>
                ))}
              </tbody>
            </table>
